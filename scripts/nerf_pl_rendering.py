...
def sample_pdf(bins, weights, N_fine, det=False, eps=1e-5):
    ...
    Inputs:
        bins: (N_rays, N_coarse_+1) where N_coarse_ is "the number of coarse samples per ray - 2"
        weights: (N_rays, N_coarse_)
    ...
    """
    N_rays, N_coarse_ = weights.shape
    weights = weights + eps # prevent division by zero (don't do inplace op!)
    pdf = weights / reduce(weights, 'n1 n2 -> n1 1', 'sum') # (N_rays, N_coarse_)
    cdf = torch.cumsum(pdf, -1) # (N_rays, N_coarse), cumulative distribution function
    cdf = torch.cat([torch.zeros_like(cdf[: ,:1]), cdf], -1)  # (N_rays, N_coarse_+1) 
                                                               # padded to 0~1 inclusive

    if det:
        u = torch.linspace(0, 1, N_fine, device=bins.device)
        u = u.expand(N_rays, N_fine)
    else:
        u = torch.rand(N_rays, N_fine, device=bins.device)
    u = u.contiguous()
...
def render_rays(models,
    ...
    def inference(results, model, xyz, z_vals, test_time=False, **kwargs):
        ...
        if typ=='coarse' and test_time:
            ...
            static_sigmas = rearrange(out, '(n1 n2) 1 -> n1 n2', n1=N_rays, n2=N_coarse_)
            ...
            out = torch.cat(out_chunks, 0)
            out = rearrange(out, '(n1 n2) c -> n1 n2 c', n1=N_rays, n2=N_coarse_)
            static_rgbs = out[..., :3] # (N_rays, N_coarse_, 3)
            static_sigmas = out[..., 3] # (N_rays, N_coarse_)
            if output_transient:
        ...
        deltas = z_vals[:, 1:] - z_vals[:, :-1] # (N_rays, N_coarse_-1)
        delta_inf = 1e2 * torch.ones_like(deltas[:, :1]) # (N_rays, 1) the last delta is infinity
        deltas = torch.cat([deltas, delta_inf], -1)  # (N_rays, N_coarse_)
...
#             noise = torch.randn_like(static_sigmas) * noise_std
    ...
    if perturb > 0: # perturb sampling depths (z_vals)
        z_vals_mid = 0.5 * (z_vals[: ,:-1] + z_vals[: ,1:]) # (N_rays, N_coarse-1) interval mid points
