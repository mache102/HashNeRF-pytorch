...
def sample_pdf(bins, weights, fine_samples, det=False, eps=1e-5):
    ...
    pdf = weights / reduce(weights, 'n1 n2 -> n1 1', 'sum') # (N_rays, coarse_samples_)
...
def render_rays(models,
    ...
    def inference(results, model, xyz, z_vals, test_time=False, **kwargs):
        ...
        Inputs:
            ...
            xyz: (N_rays, coarse_samples_, 3) sampled positions
            ...
            z_vals: (N_rays, coarse_samples_) depths of the sampled positions
        ...
        if typ=='coarse' and test_time:
            ...
            static_sigmas = rearrange(out, '(n1 n2) 1 -> n1 n2', n1=N_rays, n2=coarse_samples_)
            ...
            out = torch.cat(out_chunks, 0)
            out = rearrange(out, '(n1 n2) c -> n1 n2 c', n1=N_rays, n2=coarse_samples_)
            static_rgbs = out[..., :3] # (N_rays, coarse_samples_, 3)
            static_sigmas = out[..., 3] # (N_rays, coarse_samples_)
            if output_transient:
        ...
        deltas = z_vals[:, 1:] - z_vals[:, :-1] # (N_rays, coarse_samples_-1)
        delta_inf = 1e2 * torch.ones_like(deltas[:, :1]) # (N_rays, 1) the last delta is infinity
        deltas = torch.cat([deltas, delta_inf], -1)  # (N_rays, coarse_samples_)
...
#             noise = torch.randn_like(static_sigmas) * noise_std
    ...
    z_vals = z_vals.expand(N_rays, coarse_samples)
    
    if perturb > 0: # perturb sampling depths (z_vals)
        z_vals_mid = 0.5 * (z_vals[: ,:-1] + z_vals[: ,1:]) # (N_rays, coarse_samples-1) interval mid points
